...
export const migrationsSchemaVersion43 = (oldRealm: Realm, newRealm: Realm) => {
  if (oldRealm.schemaVersion < 43) {
    const type: WalletType = 'igraCanonical';
    ...
    for (const account of accounts) {
      ...
      const existingWallet = newRealm.objects<RealmWallet>(REALM_TYPE_WALLET).filtered(`type == '${type}' && accountIdx == ${accountIdx}`)[0];
      if (existingWallet) {
        continue;
      }
